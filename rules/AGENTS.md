...
- `--device auto` / `--host auto` lets the CLI pick from your registered fleet using live headroom + canonical account eligibility. The local machine is one candidate, not a fallback: if every candidate is ineligible or placement cannot be evaluated, the command fails loud. Use this as the default for "send to the fleet"; use a named `--device <box>` only when the task must land on a specific machine.
