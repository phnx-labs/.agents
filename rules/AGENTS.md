...
immediately without review or CI, except when the diff includes security-critical
...
Reuse and enrich existing tickets; create one when the work warrants it, keep
its description and status current, and close it with evidence. Incidental
findings go in the owner update. Labels, cycles, milestones, and projects are
the owner's taxonomy: pick from what exists, do not add to it. The `tickets`
