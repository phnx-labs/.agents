...
- **Env vars are ambient global state, not a secure boundary and not config.** Children inherit the full environment, same-uid processes can read `/proc/<pid>/environ`, and values leak into crash dumps and logs — treat anything in one as visible. Feature flags, endpoints, and behavior toggles go in `agents.yaml` / project config, never an env var that silently shadows what the config file says. Before creating a new one, ask whether a config entry, a CLI flag, or a function argument would do — each ad-hoc env var is a hidden configuration surface nobody reading the config expects.
