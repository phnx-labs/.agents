...
- **Balanced rotation is already the default.** A bare teammate (no `@version` or `--profile`) inherits the `balanced` strategy: weighted-random across healthy accounts by remaining headroom, skipping rate-limited ones. Pinning a version or profile opts that teammate out, on purpose — pin only when it genuinely needs a specific version or identity. Don't add your own rotation logic on top.
