...
that lost time. Do not add to that number. Agents' own words, verbatim, each
costing hours of waiting:
...
- **Owner escalation is the LAST resort, strictly ordered:** iMessage → voice call (blocking-prod only). Keep every other thread moving meanwhile; never idle. Owner-contact does **not** override the F5 irreversible-escalation gates.
...
- **Diagnose against live code, not a stale checkout.** `git fetch origin` and check how far behind you are before you call something a bug or open a "fix" — a fix built on stale code is itself the regression. See `research-discipline` (current-code anchoring) and `truly-agentic-git-workflow`.
...
- **A message or application is done only when it is staged where it gets sent** — a reply draft inside the Gmail/InMail thread, the LinkedIn or application composer with the text already in it. Draft text in a scratch `.md` file or on the clipboard is NOT "send-ready" and must not be called that. Stage it in the channel; the only piece you legitimately hand back is the final Send when it genuinely requires the user's own identity.
...
- **If it needs to reach their phone** (they may be away), also send the out-of-band owner notification with the link (`agents notify` — see `notify-owner`); the harness only notifies *you*, never them. Keep it short: **1–4 lines**, lead with the one thing you need, the text is a pointer (link the PR/ticket), not the payload.
...
- **A session transcript is confidential — always.** It can carry secrets, tokens, internal paths, and raw reasoning. Never inline it in a PR/issue/ticket body, never on a public repo or public tracker. Private repo: attach as a **secret gist** and link only. Public repo: omit it, reference the local `<host>:<path>` instead. The single narrow exception is `agents sessions share <id>` (see `truly-agentic-git-workflow`).
