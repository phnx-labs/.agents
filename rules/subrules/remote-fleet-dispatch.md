...
- `--device auto` lets the CLI pick from the automatic-placement pool. For a named harness it prefers a device with a ready account, then lower live load. A trailing-`@` account picker also admits devices with a selectable login target but excludes throttled-only devices. Use this as the default for "send to the fleet"; use a named `--device <box>` only when the task must land on a specific machine.
