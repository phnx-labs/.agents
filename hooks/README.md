...
| Direct-file guard | Its anchored matcher covers canonical `Read`/`Edit`/`Write`, OpenCode native `read`/`edit`/`write`, `ReadFile`/`read_file`, `edit_file`/`search_replace`/`MultiEdit`, `write_file`, and Codex `apply_patch`; Bash remains outside the registered boundary. Native lowercase calls map to the same canonical operations. It accepts `file_path`, `filePath`, and `path` under either `tool_input` or `toolInput`, but rejects unequal aliases instead of choosing one. Raw stdin is captured in a private runtime file with trapped cleanup; NUL bytes are rejected before the validated text enters a shell variable. A shared raw-JSON preflight then rejects duplicate decoded member names at any nesting depth and enforces one parser-independent numeric contract before jq, Node, or Python extraction: every JSON number in a matched payload must be a finite integer from `-9007199254740991` through `9007199254740991`; signed zero is accepted as zero, while fractions, larger integers, NaN, and infinities fail closed. Within that domain, booleans remain distinct from numbers and recursive arrays and objects compare structurally. For `apply_patch`, it parses the raw `command` and checks every Add, Update, Delete, source, and move target. Invalid, unknown, malformed, and pathless matched calls fail closed. Protected paths come from `permissions/groups/99-deny.yaml`; recognized Windows drive and MSYS home forms normalize before comparison, while backslashes in POSIX paths remain literal. Lexical matching uses a collapsed absolute path, while metadata-only physical projection receives the original uncollapsed absolute path and resolves through the nearest existing ancestor; this preserves kernel symlink-plus-`..` semantics, allows safe symlinks, and denies symlinks into protected paths. |
...
| [`12-direct-file-credential-guard.sh`](./pre-tool-use/12-direct-file-credential-guard.sh) | Blocks canonical and OpenCode native lowercase read/edit/write aliases plus Codex raw `apply_patch` operations whose lexical or physical targets match canonical `permissions/groups/99-deny.yaml`; rejects type-distinct conflicting aliases and malformed/pathless patches while allowing Bash, literal-backslash POSIX paths, safe symlinks, and unrelated repository-local paths |
