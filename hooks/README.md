...
(e.g. `~/.linear-cli/config.json`). Never `agents secrets` (or bare `secrets`) from a hook, and never
...
| Hook | What it does |
|---|---|
| [`02-worktree-reclaim-nudge.py`](./post-tool-use/02-worktree-reclaim-nudge.py) | Advisory: after a **successful** `gh pr merge`, reminds the agent to remove the merged worktree with plain `git -C <repo> worktree remove <path>` — which git-guard already permits on a clean tree (PHNX-3503). Never removes anything itself; exit 0, fails open |
