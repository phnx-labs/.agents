...
`agents logs [id]` is the unified viewer over device-dispatch runs and local session transcripts; scope it to one machine with `--host <name>`. See `agents devices --help`.
