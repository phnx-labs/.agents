...
8. **Deliver it to the user's machine.**
   ...
   Resolve the online macOS device from the Host & Fleet context (never hardcode). Copy the HTML over, then show it in **one reused browser tab** with `agents browser navigate` — re-presenting an updated visual refreshes the SAME tab in place instead of piling up a new duplicate tab every time (a raw `open` opens a fresh tab per call):
