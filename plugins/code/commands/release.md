...
- Refuses to become a *second* releaser when one is verifiably mid-flight — a held `release-lease.sh`, an open `chore(release)` PR with a live driver, a running `release.sh`, or a CI workflow that publishes on the very tag/push it is about to make — and coordinates with that one instead, verifying the registry flipped either way. Nothing auto-ships a merge, so it never defers to a *scheduled* releaser, and a dead or closed release is not a reason to stop.
