...
- Refuses to become a *second* releaser when one is already mid-flight (a held `release-lease.sh`, an open `chore(release)` PR, a running `release.sh`) — coordinates with that one instead. Nothing auto-ships a merge, so it never hands off to a scheduled releaser.
