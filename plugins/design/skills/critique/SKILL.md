---
name: critique
description: "Audit an existing design surface — a URL, local HTML file, screenshot, several pages of one site, or a running app — and return ranked, concrete findings. Deterministic checks (WCAG contrast computed, never guessed; an anti-tells/offline/color-only-status linter) plus a visual pass on real screenshots, diffed against the product's own brand tokens and against each other for drift. Outputs a paste-ready fix brief and standing design laws for the project's docs. Triggers on: critique this design, audit this screen/site/app, is this design any good, find design inconsistencies, review this UI."
allowed-tools: Bash(agents browser*), Bash(agents computer*), Bash(bun*), Bash(curl*), Read, Write
user-invocable: true
---
...
Read [`design-core.md`](../design/design-core.md). Its 11-item checklist (§9) is the
rubric; the anti-tells catalog (§6) and the accessibility rules (§3) are the two sections
the scripts mechanize.

## When to use (vs neighbors)

- Judge, audit, or find inconsistencies in something that exists → **critique** (this).
- Then rebuild a screen from the findings → **`design:prototype`** (redesign path).
- Building or refining a token/component system, not judging one → **`design:system`**.
- Building or refining a standalone graphic → **`design:graphics`**.
...
4. **Check against the product's own system.** Probe for `BRAND.md`, `DESIGN.md`, design
   tokens, or a tailwind theme (design-core §4). A page that disagrees with its own
   product's tokens — a stray font, an off-palette accent, a third spacing scale — is a
   finding even when it looks fine in isolation. On a multi-page audit, build a small
...
6. **Route the findings so they stay fixed.** Deliver both, then offer the third:
   - **Fix brief** — a paste-ready block the user can hand to any building agent (or you
     dispatch yourself via `agents run` / the `design:prototype` skill): one bullet per
     finding, imperative, with the file/selector and the target value. No context-hunting
     required by the receiving agent.
   ...
   - **Rebuild** — offer to run `design:prototype` (redesign path) on the worst screen.
...
- For a standing audit the user will share, render the report with the `artifacts` skill
  (self-contained HTML) and show it via `agents browser show` (see the `browser` skill) —
  do not hardcode `open`/`xdg-open` or a `/tmp`/`~/Downloads` default.
