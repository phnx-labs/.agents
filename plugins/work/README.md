...
| Command | Use when |
| --- | --- |
| `/work:dispatch` | You have ONE unit of work — a ticket, a described task, or "the next thing on `<project>`" — to get done by the right agent on the right machine. Finds/pulls the ticket (deduping against in-flight work), classifies it as coding vs non-coding, files it clean if needed, then self-refers to the plugin that owns the execution (`code` for engineering; `social`/`design`/`create`/`browser` for non-coding) and drives it to done. Single-target; **not** a board sweep. |

## How it relates to the neighbours

- **`/code:dispatch`** is the engineering-only counterpart. `/work:dispatch` is its superset: same shape, but a work item can be non-coding, and it routes across plugins rather than assuming a code change.
...
- **Non-coding is first-class.** A blog post, a creator email, a funnel pull, an OG image, a portal task — all are work items an agent can *do* here via the `social`/`design`/`create`/`share` plugins + the `browser` skill + `secrets`.
