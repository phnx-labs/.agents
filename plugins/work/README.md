...
| `work:research` | Multi-modal research: frame the question into angles, fan out across engines that each see a different slice (Codex/web, Grok/X, Antigravity/Google, Perplexity/broad Deep Research, Claude/deep-read) **blind to each other**, cross-check claims across sources (single-sourced = a lead to verify), and synthesize one cited artifact into `docs/research/`. Composes `run` + `teams` + `browser` + `artifacts`. |
