...
couldn't grep it." Transcript recovery is strongest for Claude Code and Droid
(full user/assistant/tool extraction), Codex (user/assistant/tool extraction;
its auto-injected `developer`-role context is recovered too, bucketed as
`tool`), and Grok (reads the sibling `chat_history.jsonl` next to the indexed
`summary.json`). Kimi's transcript is split across several per-agent files
recall.py does not open yet, so Kimi hits are index-only until that lands.
This never affects layers 1-2 or the
