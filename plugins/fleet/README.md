...
| `/fleet:mint-auth [account]` | **Authorize harness credentials yourself** — no user hand-off. Prefers a fresh native device/OAuth login inside each target's stable account slot, verified by the resulting email; Grok/Kimi/Droid native credentials are minted independently per device and never copied. Claude setup tokens and API keys remain the deliberately syncable alternative: store them as named provider accounts and distribute only through `agents accounts sync`. |
