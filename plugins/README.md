...
|---|---|---|
| [`code`](./code/README.md) | 7 | The coding loop — `/code:loop`, `/code:review` (session PRs / cold PR review / whole-repo scan, three modes on one skill), `/code:learn` (writes project AGENTS.md nav notes), `/code:refactor` (architectural restructuring), `/code:score` (agent-readiness score, `AGENTS.md` coverage, directory-mess visual report), and `/code:commit`. Plus one self-contained command — `/code:prune` (remove merged branches/worktrees behind data-loss guards). `/code:review` spawns the repo's [`code-reviewer`](../subagents/code-reviewer/AGENT.md) subagent, which ships from `subagents/`, not from this plugin. |
| [`work`](./work/README.md) | 4 | General-purpose work — `/work:loop` unattended multi-project drain with load spread + browser/computer; `/work:dispatch` is ONE unit of work (coding or not); `/work:triage` is the board-wide keep/cancel sweep; `/work:output` is the fleet token-burn/output report |
