...
|---|---|---|
| [`code`](./code/README.md) | 8 | The coding loop — `/code:loop`, `/code:review` (session PRs / cold PR review / whole-repo scan, three modes on one skill), `/code:learn` (writes project AGENTS.md nav notes), `/code:commit`, `/code:release` (publish a package/CLI/app to its registry — discover release process, run tests, changelog, publish, tag, verify live). Plus three self-contained commands — `/code:prune` (remove merged branches/worktrees behind data-loss guards), `/code:tag-release` (cut and push an annotated release tag), `/code:clean` (clean up tech debt, duplicates, dead code) — moved here when the standalone `git` plugin and top-level `/clean` were retired. `/code:review` spawns the repo's [`code-reviewer`](../subagents/code-reviewer/AGENT.md) subagent, which ships from `subagents/`, not from this plugin. |
