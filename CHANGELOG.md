...
- **Worktrees are reclaimed after merge, on every device (PHNX-3503).** Worktree law created a checkout per change and nothing ever removed it — `gh pr merge --delete-branch` drops the branch and leaves the tree — so merged worktrees accumulated until a box ran out of disk (581 worktrees / ~263 GB measured across the fleet; the release home base wedged at 1.6 GiB free, PHNX-3478). Two layers, both delegating their safety to `agents worktree` (agents-cli 1.22.61+), which fails closed and refuses anything dirty or not fully landed:
  ...
  - `routines/worktree-sweep.yml` — daily 04:30 backstop that sweeps each box's own repos, **discovering** the worktree containers on the box rather than assuming a fixed root list. **Deliberately no `devices:` restriction**, exactly like `check-updates.yml`: per §Scheduling & execution singularity an unrestricted routine may fire everywhere when its input is the firing device's own state, and worktree cleanup is device-local. Pinning it to one device would clean that box and leave every other one leaking, which is the failure being fixed.

  Review follow-ups on this change, all in the routine rather than the hook: its reclaimed-count parser could never match (`grep -o '"reclaimed":\[...\]'` is line-based, but `agents worktree sweep --json` pretty-prints, so the count was **always 0** — every box would have reported "0 reclaimed" nightly while genuinely deleting worktrees, masking both real activity and any regression in the CLI's fail-closed logic); its four hardcoded roots found **14** worktree containers on yosemite-s1 where discovery finds **82**, so ~83% of the leak — everything under `~/.agents/repos`, `~/.agents/.system`, `~/agents-cli` and `$HOME` itself — was never going to be swept, and three of those four roots exist nowhere on this fleet; and the bespoke `AGENTS_WORKTREE_GRACE_DAYS` env var was dropped in favour of the repo's existing per-device override (a same-named file in `~/.agents/routines/`).

  A second review round found two more, both blocking: the routine **could never run at all** — `command:` routines execute via `/bin/sh -c`, which is `dash` on the Linux boxes, and `done < <(find ...)` is a bashism, so dash aborted with `Syntax error: redirection unexpected` (exit 2, zero repos) on every scheduled fire since the original commit; and the widened discovery matched directory *shape* rather than "a repo this fleet manages", so a nightly destructive job could reach any checkout that happens to use worktree law. The loop now materialises the list through a temp file (a pipe would not do — POSIX runs the loop body in a subshell, so the counters would be discarded and the summary would always print 0), and a repo can opt out with a `.no-worktree-sweep` marker at its root or an exact path in `~/.agents/worktree-sweep.exclude`.
