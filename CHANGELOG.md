...
- **Default and sandbox permissions now share one explicit wide-access contract.** Both
  manifests include the existing `12-self` group, drop the nonexistent `16-mcp` group,
  blanket-allow `Bash`, `Read`, `Write`, and `Edit`, and carry the same twelve-path
  credential file-tool deny matrix. `build.sh` now follows `presets/default.yaml` instead
  of globbing a different group set, and `permissions_test.sh` locks manifest parity,
  generated bytes, exact deny entries, and unchanged existing destructive guard files and
  registrations. These rules remove ordinary harness approval prompts, but command-substitution/
  simple-expansion and cd-before-git prechecks can still prompt first. Credential denies reduce accidental
  file-tool access; allowed Bash can still reach those paths. Those existing destructive hooks
  remain cooperative guardrails, not an adversarial or operating-system security boundary.
