...
- **The `code-reviewer` subagent reviews against intent and the surface's design, in a bounded radius.** Four changes from watching it run: (1) it now loads the **requirement** before judging — `code:review` B1b resolves the ticket (`RUSH-1234` from the branch/title/body) and any committed plan, quotes the acceptance criteria into the brief, and the reviewer answers conformance per criterion first; a reviewer with no requirement can only grade code against itself. (2) A new hunt class, **design divergence at a declared surface**: a new endpoint, CLI command, schema, or component is diffed against three to five existing siblings (API envelope/error shape/pagination/auth attachment; CLI noun-then-verb and `--json`; UI tokens, scale, component reuse, full state set) — "it works" is not the bar when the divergence is what future callers pay for. (3) An explicit **finding radius**: the diff, plus what the diff makes wrong (stale callers, disagreeing siblings, and the old path this change orphaned but nobody deleted), and pre-existing rot is out of scope with a one-line note. The test is whether reverting the diff would fix it. (4) The **security pass is its own agent** spawned alongside the reviewer instead of check #6 in its brief — concurrent, so it costs no wall-clock, and it stops the security tail getting the least attention. Source: `plugins/code/agents/code-reviewer.md`, `plugins/code/skills/review/SKILL.md`.
