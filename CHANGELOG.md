...
- **Promptcuts and bangcuts are enabled by default and use stable public names (RUSH-2405).** `promptcuts` expands saved prompt shortcuts; `bangcuts` executes explicit backticked bang commands. Users can enable or disable either feature with `agents hooks enable|disable promptcuts|bangcuts`. The internal manifest keys remain `expand-promptcuts` and `expand-bang-commands` so existing user-layer YAML keeps working. Source: `agents.yaml`, `hooks/README.md`, `hooks/promptcuts.yaml`.
