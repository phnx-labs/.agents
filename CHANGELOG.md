...
- **`code-quality` rule: "Delete what you supersede, in the same change."** New bullet in `rules/subrules/code-quality.md` (re-composed into the standing `rules/AGENTS.md`, which every agent gets as its memory file). The rule already governed how agents *add* — "Fewer concepts beat more code", "No duplicate code", "Cross-cutting changes go to the source" — but nothing told an agent to *remove the path its own change obsoletes*, so agents accrete: they wire in the new function/flag/command and leave the old one standing "for later". The tell is file and concept count climbing with no new capability — the exact rot that makes a codebase like `agents-cli` hard to navigate. `code-reviewer` already flags orphaned code at merge time (finding radius, ring 2), and `/code:refactor` remediates a repo that has already rotted; this makes the net-negative-diff bias an **authoring** instruction so the sprawl is never written in the first place. Explicitly scoped as *finishing your own change, not scope creep*, to sit cleanly beside the "No scope creep" bullet.
