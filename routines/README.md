...
| [`worktree-sweep`](./worktree-sweep.yml) | daily 04:30 | Reclaims PR-bound worktrees whose work has landed, on every device — `agents worktree sweep` removes a merged worktree **and** its branch, fails closed on anything unlanded (PHNX-3503). Deliberately unpinned: its input is the firing box's own checkouts. Opt a repo out with a `.no-worktree-sweep` file at its root, or its exact path in `~/.agents/worktree-sweep.exclude` |
